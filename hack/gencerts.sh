...
# Set the namespace to "kube-system" by default if not provided.
# Set the webhook service name to "kubernetes-webhook-injector" by default if not provided.
IN_POD=false
SERVICE="kubernetes-webhook-injector"
NAMESPACE="kube-system"
parse_arguments "$@"

TMP_DIR="/tmp/certs"

echo "Generating certs for the kubernetes-webhook-injector pod admission webhook in ${TMP_DIR}."
mkdir -p ${TMP_DIR}
...
if [[ "$IN_POD" == "true" ]];  then
  ...
  kubectl create secret --namespace=${NAMESPACE} generic kubernetes-webhook-injector-certs --from-file=${TMP_DIR}/ca-key.pem --from-file=${TMP_DIR}/ca-cert.pem --from-file=${TMP_DIR}/server-key.pem --from-file=${TMP_DIR}/server-cert.pem
fi
